...
def runner():
 ...
 #create X_{T-1} to X_T
    ...
    # Sample array to train global gp
    control_inputs = np.zeros((0,3))
    control_outputs = np.zeros(0)
    ...
    for iStep in range(nstep,0,-1):
        ...
        # save training samples at each time step
        n_samples = len(costNext)
        cur_time = np.ones(n_samples) *t[iStep-1]
        time_demand_soc = np.column_stack((cur_time,demandMatrix[:,1],I_next))
        control_inputs  = np.row_stack((control_inputs,time_demand_soc))
        control_outputs = np.concatenate((control_outputs,optimal_B))


    return Model,control_inputs,control_outputs
